...
class Game(arcade.View):
    ...
    def on_draw(self):
        ...
        if self.game_pause or not self.game_state:
            if not self.game_state and self.game_pause:
                arcade.draw_text("GAME PAUSED", 220, 350, arcade.color.WHITE, 55)
            else:
...
class Control_View(arcade.View):
    def __init__(self):
        super().__init__()
    ...
    def on_draw(self):
        arcade.start_render()
        arcade.draw_text("CONTROLS", SCREEN_WIDTH / 2, SCREEN_HEIGHT - 100, arcade.color.WHITE, 30, anchor_x="center")
        arcade.draw_text("Use the LEFT and RIGHT arrow keys to move", 50, SCREEN_HEIGHT - 200, arcade.csscolor.WHITE,
                         15, anchor_x="left")
        arcade.draw_text("Use the SPACE BAR to shoot laser", 50, SCREEN_HEIGHT - 250, arcade.csscolor.WHITE, 15,
                         anchor_x="left")
        arcade.draw_text("Use ESC key to pause the game and also to unpause the game", 50, SCREEN_HEIGHT - 300,
                         arcade.csscolor.WHITE, 15, anchor_x="left")
        arcade.draw_text("Use Q key to quit the game. Only applicable when the game is paused", 50, SCREEN_HEIGHT - 350,
                         arcade.csscolor.WHITE, 15, anchor_x="left")
        arcade.draw_text("Press ESC key to go back", 20, 50, arcade.csscolor.WHITE, 12, anchor_x="left")

    def on_key_press(self, key, modifiers):
        if key == arcade.key.ESCAPE:
            control_go_back = HomeView()
            self.window.show_view(control_go_back)


class Help_View(arcade.View):
    def __init__(self):
        super().__init__()
    ...
    def on_draw(self):
        arcade.start_render()
        arcade.draw_text("WELCOME TO SPACE INVADERS", SCREEN_WIDTH / 2, SCREEN_HEIGHT - 100, arcade.color.WHITE, 30,
                         anchor_x="center")
        arcade.draw_text(
            "Space  invader is a single person arcade shooting game which comprises of the player being in control of the ",
            20, SCREEN_HEIGHT - 200, arcade.color.WHITE, 12, anchor_x="left")
        arcade.draw_text(
            "spaceship blasting his way through an endless stream of flying enemies towards glory advancing in level and",
            20, SCREEN_HEIGHT - 230, arcade.color.WHITE, 12, anchor_x="left")
        arcade.draw_text(
            "showing his fast response time. Be ready to save your people and be called the hero of the galaxy. It is your",
            20, SCREEN_HEIGHT - 260, arcade.color.WHITE, 12, anchor_x="left")
        arcade.draw_text(
            "chance to show your skills and be the best fighter of the universe. Get suited and booted for an exciting",
            20, SCREEN_HEIGHT - 290, arcade.color.WHITE, 12, anchor_x="left")
        arcade.draw_text("gaming experience which is bound to bring back old memories from your childhood.", 20,
                         SCREEN_HEIGHT - 320, arcade.color.WHITE, 12, anchor_x="left")
        arcade.draw_text("Press ESC key to go back", 20, 50, arcade.csscolor.WHITE, 12, anchor_x="left")

    def on_key_press(self, key, modifiers):
        if key == arcade.key.ESCAPE:
            help_go_back = HomeView()
            self.window.show_view(help_go_back)
...
class HomeView(arcade.View):
    ...
    def on_draw(self):
        arcade.start_render()
        arcade.draw_text("SPACE INVADERS", SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, arcade.color.WHITE, 50,
                         anchor_x="center")
        arcade.draw_text("Press space bar to start the game", self.window.width / 2, self.window.height / 2 - 75,
                         arcade.color.WHITE, 15, anchor_x="center")
        arcade.draw_text("C - CONTROLS", 100, 50, arcade.color.WHITE, 15, anchor_x="center")
        arcade.draw_text("H - HELP", self.window.width / 2, 50, arcade.color.WHITE, 15, anchor_x="center")
        arcade.draw_text("Q - QUIT", self.window.width - 100, 50, arcade.color.WHITE, 15, anchor_x="center")
    ...
    def on_key_press(self, key, modifiers):
        ...
        if key == arcade.key.H:
            help_desk = Help_View()
            self.window.show_view(help_desk)

        if key == arcade.key.C:
            control_desk = Control_View()
            self.window.show_view(control_desk)
